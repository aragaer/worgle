...
The file name is currently acquired from the command line, so the
...
#+END_SRC
***** Unnamed block
It is possible a block has no explicit name specified.
#+NAME: find_next_unnamed_block
#+BEGIN_SRC c
if (
    <<begin_src_check>>
    ) {
...
rc = 1;
#+END_SRC
***** Condition to check the beginning of a source block
#+NAME: begin_src_check
#+BEGIN_SRC c
read >= 11 && !strncasecmp(line, "#+BEGIN_SRC",11)
#+END_SRC
***** Process the source block declaration
#+NAME: begin_src_found
#+BEGIN_SRC c
<<begin_the_code>>
...
#+END_SRC
***** Starting an anonymous block
If block has no explicit name, start a new anonymous block.
#+NAME: begin_the_code
#+BEGIN_SRC c
if(!worg->curblock) worgle_begin_block(worg, &anonymous);
#+END_SRC

#+NAME: global_variables
#+BEGIN_SRC c
struct worgle_string anonymous = {"",0};
#+END_SRC
...
A worgle code block is initialized using the function
=worgle_block_init=.
...
A generic segment is appended to a code block with the
function =worgle_block_append_segment=.

The block =b=, name of the block =str=, and type =type= are
mandatory parameters which describe the segment. The
location in the file is also required, so the line number
=linum= and name of file =filename= are also provided as
...
In both CWEB and Org-tangle, existing code blocks can be
appeneded to in different sections. Because of how this
program works, this functionality comes for free.

It can be useful to be able to differentiate between
different code block sections. This is done using a variable
called =nblocks=, which keeps track of the number of blocks
in a code block.
...
Every time a new =NAME='d block appears,
the =nblock= variable gets incremented. The current =nblock=
value is used as a position value that gets stored in the
=worgle_orglet_blkref=.


#+NAME: increment_nblocks
#+BEGIN_SRC c
...
#+END_SRC
*** Keeping track of the last segment
The id of the last segment appended is kept track of. This
done for the benefit of =worgle_orglet_blkref=.
...
When a new block reference is created, the id of the last
...
To ensure that org content for a program spread out multiple
files get linked properly, a =prev= file entry is utilized.
If it isn't NULL, the =tail= of the previous list is
linked to the =head= of the current list.
...
The integer variables here take a snapshot of current
code block, and can be used to recreate the code inside a
code subblock.

=pos= is derived from the =nblocks= counter, and is turned
into a relative position id.

=prev_lastseg= is the id of last segment of the previous
block, and is negative if it doesn't exist. Peaking at the
next id of this segment gets the first segment of the
subblock.

=segoff= is the segment offset. It stores the starting
segment position relative to the block. Subtracting the
=segoff= of the next subblock obtains the number of segments
...
#+END_SRC
* Source mapping
Worgle has the ability to produce Org data that outlines the
...
Every file has a top level block, which is recursively
iterated through.
